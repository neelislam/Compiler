...
bool F() {
    string tok = nextToken();   // Get next token
    return tok == "id";         // Return true if token is 'id', else false
}

// Main function
int main() {
    input = "id+id*id";  // Set the input string to parse
    pos = 0;             // Start from the beginning

    // Parse the expression using E and check if entire input is consumed
    if (E() && pos == input.length()) {
        cout << " Valid string according to the grammar." << endl;  // Success
    } else {
